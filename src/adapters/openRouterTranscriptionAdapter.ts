...
export type OpenRouterTranscriptionRequest = (input: { filePath: string; model: string; apiKey: string; requestTimeoutMs: number }) => Promise<{ text?: unknown }>;
...
export class OpenRouterTranscriptionAdapter implements TranscriptionAdapter {
  ...
  constructor(private readonly config: { apiKey: string; model?: string; request?: OpenRouterTranscriptionRequest; requestTimeoutMs?: number }) {
  ...
  async transcribe(input: TranscriptionInput): Promise<TranscriptionResult> {
    ...
    for (const chunk of chunks) {
      const response = await this.request({ filePath: chunk.path, model: this.model, apiKey: this.config.apiKey, requestTimeoutMs: this.config.requestTimeoutMs ?? defaultProviderRequestTimeoutMs });
...
async function requestOpenRouterTranscription(input: { filePath: string; model: string; apiKey: string; requestTimeoutMs: number }): Promise<{ text?: unknown }> {
  const audio = await readFile(input.filePath);
  const response = await fetchWithProviderTimeout(fetch, "https://openrouter.ai/api/v1/audio/transcriptions", {
    method: "POST",
