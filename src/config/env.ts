...
export function loadConfig(env: NodeJS.ProcessEnv): AppConfig {
  ...
  return {
    botToken, allowedTelegramIds, databaseUrl,
...
export function parseTelegramIdAllowlist(raw: string): Set<string> {
  ...
  if (ids.length === 0) throw new Error("ALLOWED_TELEGRAM_IDS must contain at least one Telegram id.");
  for (const id of ids) if (!/^\d{1,20}$/.test(id)) throw new Error("ALLOWED_TELEGRAM_IDS must be a comma-separated list of numeric Telegram ids.");
  return new Set(ids);
}
function readRequired(env: NodeJS.ProcessEnv, name: string): string { const value = env[name]?.trim(); if (!value) throw new Error(`${name} is required.`); return value; }
function readOptional(env: NodeJS.ProcessEnv, name: string): string | undefined { const value = env[name]?.trim(); return value || undefined; }
function readOptionalInteger(env: NodeJS.ProcessEnv, name: string): number | undefined { const raw = readOptional(env, name); if (raw === undefined) return undefined; const value = Number(raw); if (!Number.isSafeInteger(value) || value <= 0) throw new Error(`${name} must be a positive integer.`); return value; }
function readOptionalBoolean(env: NodeJS.ProcessEnv, name: string): boolean | undefined { const raw = readOptional(env, name); if (raw === undefined) return undefined; if (raw === "true") return true; if (raw === "false") return false; throw new Error(`${name} must be true or false.`); }
