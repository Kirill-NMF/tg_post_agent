...
export function buildApplication(env: NodeJS.ProcessEnv) {
  ...
  const projectService = new ProjectService(repository, new MockModelAdapters(), jobRepository, { sourceAudio: config.sourceAudioJobMaxAttempts, editAudio: config.editAudioJobMaxAttempts, planRevision: config.planRevisionJobMaxAttempts });
...
function createWorkerRuntime(input: {
  ...
  if (!input.config.openRouterApiKey && !input.config.openaiApiKey) {
    throw new Error("OPENROUTER_API_KEY or OPENAI_API_KEY is required when JOB_WORKER_ENABLED=true.");
  }
  if (!input.config.openRouterApiKey && !input.config.geminiApiKey) {
