...
export function createTranscribeEditAudioJobHandler(deps: {
  ...
  return async (job: Job) => {
    if (job.type !== "TRANSCRIBE_EDIT_AUDIO" || !job.projectId) throw new PermanentJobError("INVALID_EDIT_AUDIO_JOB", "Edit-audio job is invalid.");
    ...
    try {
      ...
      const downloaded = await deps.telegramFiles.downloadFile({ fileId: payload.source.telegramFileId, targetPath: workspace + "/source" });
      const prepared = await deps.audioProcessor.prepareForTranscription({ sourcePath: workspace + "/source", workspaceDir: workspace });
      const result = await deps.transcription.transcribe({
        projectId: project.id,
        jobId: job.id,
        source: { ...payload.source, sizeBytes: payload.source.sizeBytes ?? downloaded.sizeBytes, durationSeconds: payload.source.durationSeconds ?? prepared.probe.durationSeconds },
        chunks: prepared.chunks
      });
      const latestUserEdit = result.transcript.trim();
      if (!latestUserEdit || latestUserEdit.length > 2000) throw new PermanentJobError("EDIT_TRANSCRIPT_INVALID", "Voice edit transcript is invalid.");
      if (!project.isActive || project.state !== payload.stateAtEdit) {
        throw new PermanentJobError("EDIT_AUDIO_STALE", "Project state changed before the voice edit could be applied.");
      }
      if (payload.stateAtEdit === "planning") {
        project.messages.push({ kind: "planning_edit", text: latestUserEdit, createdAt: new Date() });
        await deps.jobs.enqueue({ type: "REVISE_PLAN", projectId: project.id, dedupeKey: "project:" + project.id + ":revise-plan:voice:" + job.id, payload: { latestUserEdit } });
      } else {
        const post = project.posts.find((item) => item.index === project.currentPostIndex);
        if (!post?.currentDraft) throw new PermanentJobError("DRAFT_CURRENT_DRAFT_MISSING", "Current draft is required before voice revision.");
        project.messages.push({ kind: "draft_edit", text: latestUserEdit, createdAt: new Date() });
        project.state = "draft_generating";
        await deps.jobs.enqueue({ type: "REVISE_DRAFT", projectId: project.id, postId: post.id, dedupeKey: "project:" + project.id + ":post:" + post.index + ":revise-draft:voice:" + job.id, payload: { postIndex: post.index, latestUserEdit } });
      }
      await deps.projects.save(project);
      deps.logger?.info({ event: "edit_audio_transcribed_and_routed", jobId: job.id, projectId: project.id, stateAtEdit: payload.stateAtEdit, chunkCount: result.meta.chunkCount }, "edit audio routed");
...
function parse(payload: Record<string, unknown>): Payload {
  ...
  if (!source || typeof source !== "object" || Array.isArray(source)) throw new PermanentJobError("EDIT_AUDIO_PAYLOAD_INVALID", "Edit-audio payload is invalid.");
  ...
  return { stateAtEdit: payload.stateAtEdit, source: { kind: "edit_audio", telegramFileId: item.telegramFileId, originalFileName: optional(item.originalFileName), mimeType: optional(item.mimeType), durationSeconds: number(item.durationSeconds), sizeBytes: number(item.sizeBytes) } };
}
