...
export function createRevisePlanJobHandler(deps: {
  ...
  return async (job: Job) => {
    ...
    if (project.state !== "planning" || !project.transcript?.trim() || !project.planOptions) {
    ...
    if (!result.ok) {
      throw result.error.retryable
        ? new RetryableJobError(result.error.code, result.error.message)
        : new PermanentJobError(result.error.code, result.error.message);
    }
    ...
    return { provider: result.meta.provider, modelLabel: result.meta.modelLabel, optionCount: result.value.options.length, notificationStatus };
  };
}

function parseEdit(payload: Record<string, unknown>): string {
  if (typeof payload.latestUserEdit !== "string") throw new PermanentJobError("PLAN_EDIT_MISSING", "Plan revision payload is missing edit text.");
  const value = payload.latestUserEdit.trim();
  if (!value) throw new PermanentJobError("PLAN_EDIT_MISSING", "Plan revision payload is missing edit text.");
  if (value.length > maxEditChars) throw new PermanentJobError("PLAN_EDIT_TOO_LONG", "Plan revision edit exceeds the configured length limit.");
  return value;
}

async function notify(deps: { notifier?: TelegramNotifier; logger?: Logger }, project: Project, jobId: string): Promise<"not_configured" | "sent" | "failed"> {
  if (!deps.notifier) return "not_configured";
  try {
    await deps.notifier.sendMessage(project.chatId, renderPlanOptionsMessage(project.planOptions ?? []), { reply_markup: planReplyMarkup() });
    return "sent";
  } catch {
    (deps.logger ?? noopLogger).warn({ event: "plan_revision_notification_failed", jobId, projectId: project.id }, "plan revision notification failed");
    return "failed";
  }
}
