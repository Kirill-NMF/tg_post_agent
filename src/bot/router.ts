import type { BotResponse, FormattingOption, PlanOptionId, RewriteMode, SourceAudioInput, TelegramChatId, TelegramUserId } from "../domain/types.js";
...
export class BotRouter {
  ...
  async handleCallback(event: RouterCallbackEvent): Promise<BotResponse[]> {
    ...
    if (group === "format" && isFormattingOption(value)) return this.projects.formatCurrentPost(event.telegramUserId, value);
...
function isFormattingOption(value: string | undefined): value is FormattingOption {
  return value === "option_1" || value === "option_2";
}
