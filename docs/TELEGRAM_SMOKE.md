...
The approved one-shot source-audio run passed: canonical identity, one non-user Russian OGG/Opus upload, real transcription, and real planning response. Its category-only report confirmed local fixture and dedicated-test-project cleanup. The temporary one-attempt/no-fallback overlay was removed and the normal single worker runtime restored. This evidence does not cover real planning or draft correction; those Tier 2 regressions remain the next gate.

## Correction Canary

`pnpm run smoke:telegram-correction` is an opt-in Tier 2 harness for the remaining planning-correction routes. It creates a fixed, non-user `planning` fixture for the dedicated account only, identified by a private canary marker, without repeating source intake. It sends exactly one text correction and one generated OGG/Opus voice correction, each with one-attempt edit/revision limits and provider fallbacks disabled by a temporary runtime overlay. It accepts one plan response or one safe recovery per correction, rejects duplicate terminal responses, removes the fixture, generated audio, and its jobs, then restores normal runtime defaults. The report and daily budget ledger contain only categories, booleans, and timings.
