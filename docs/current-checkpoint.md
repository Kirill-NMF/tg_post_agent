...
The category-only provider ledger is 47/50. The first private primary-gold tuning call reached provider transport success but failed closed in the formatting adapter before candidate rendering; no quality score exists for that attempt. The ledger retains no transcript, draft, prompt, provider response, credential, or user content.
...
The approved model is `anthropic/claude-sonnet-5` through the VPS-only `OPENROUTER_FORMATTING_MODEL` path. The current owner-approved cap is 50 and the category-only ledger is 47/50. The accepted Option 2 validation used one single-attempt/no-fallback formatting call; the later private tuning attempt also used exactly one call and produced no accepted candidate. Premium/custom emoji remain deferred.
