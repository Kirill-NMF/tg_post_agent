...
| Stage 3: Formatting | Option 2 technical safety/delivery gate proven; Manus-style benchmark built; owner 3/3 pending | Shared provider/runtime schema, preservation-first FORMAT_POST, current-version final delivery, permitted emoji, and one non-empty .txt are proven. A private four-reference round-trip corpus now measures Manus/CRYPTUS structure; prompt quality has not yet passed that benchmark. | Literary quality, reference-style fit, emoji density, and UX judgement | Review the preserved Telegram result and, after prompt work, the untouched style holdout. Do not repeat technical provider validation unless behavior changes. |
...
| Phase 12: Real Formatting Adapter | Stage 3 | Option 2 Tier 1 and isolated Tier 2 technical gate green; offline Manus-style evaluator/corpus green; prompt backtest pending | 3/3 | Shared-schema contract tests, seven-segment exact-job canary, lexical/emoji/final/export checks, private gold round-trip/evaluator | Judge literary quality, emoji density, readability, reference-style fit, and first-click UX after prompt backtest. |
