...
| Phase 12: Real Formatting Adapter | Stage 3 | Option 2 delivery gate green; post-fix primary rendered but failed benchmark hard gates and style threshold; quality benchmark not passed | 3/3 | Shared dynamic schema, server-owned role/budget tests, seven-segment exact-job canary, lexical/emoji/final/export checks, private gold evaluator | Ledger 52/52; holdout remains sealed and any further provider operation requires owner decision and budget. |
...
The category-only ledger is 52/52. The post-fix primary verification used exactly one Claude Sonnet/OpenRouter attempt with fallback disabled and no retry. Shared shape and semantic gates accepted a private 38-segment candidate and preserved its lexical sequence, but the offline evaluator failed punctuation preservation and forbidden-style gates and scored 0.3874 against the 0.65 threshold. The private diff exists with owner-only permissions. `holdout_10` remains sealed because primary did not pass.
...
The role-constrained production contract and deterministic regressions are green, and the first contract-valid primary candidate now has a score. Successive category-only failures closed dynamic ID, role-to-emoji, required-role completeness, and provider/server operation-budget drift without weakening semantic safety. The remaining gate is owner/product review of the primary metric gaps, a future successful primary benchmark, untouched holdout backtesting, and owner 3/3 literary/product/UX judgement. With the ledger exhausted at 52/52, no further provider or holdout operation is authorized. The current result does not claim the Manus-style quality benchmark, all Stage 3 quality, or future changed paths are accepted.
