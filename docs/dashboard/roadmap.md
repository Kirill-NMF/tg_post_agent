...
| Stage 3: Formatting | Option 2 product correction deployed; first owner-fixture candidate failed closed | Delivery/export safety remains proven, and the fixed CRYPTUS_MEDIA final-text gate rejected a role-invalid candidate before delivery. | Inspect the preserved private candidate and decide the next prompt/provider action | Require every explicit structural rule before considering any score; literary/visual acceptance remains later. |
...
| Phase 12: Real Formatting Adapter | Stage 3 | product correction deployed; controlled call rejected safely | 3/3 | Fixed prompt, explicit structural validator, private invalid-candidate evidence, draft-preserving recovery | Ledger 56/60; inspect the private candidate and decide the next bounded correction. Do not call the style gate passed. |
...
The historical primary/holdout scores remain diagnostic evidence only. They do not satisfy the corrected explicit product contract. The first corrected owner-fixture call preserved text but failed the emoji-role, comparison-list, and final-question rules and was not delivered. The private candidate is retained for review; another provider operation requires a new bounded instruction.
