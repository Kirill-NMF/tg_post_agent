...
The role-constrained production contract and deterministic regressions are green. The remaining gate is one renewed primary model benchmark under a new owner budget; only a primary pass may unlock one untouched holdout operation. Generalization-7's 0.636 deterministic floor documents the residual need for provider-selected optional list/code/bold semantics without letting the server invent them. No model or holdout quality claim is made.
