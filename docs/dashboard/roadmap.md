...
| Phase 12: Real Formatting Adapter | Stage 3 | Option 2 delivery gate green; primary style gaps fixed offline; renewed model benchmark still pending | 3/3 | Shared schema, line-scoped roles, typed list decoration, Telegram-Markdown evaluator parity, deterministic tuning floor | Ledger 52/52; request one primary verification, then one sealed holdout only after primary passes. |
...
The category-only ledger is 52/52. The immutable primary candidate scored 0.3874. Offline TDD separated an evaluator dialect false positive from production role-inference gaps: the corrected evaluator accepts Telegram bold but still fails closed on two residual formatter symbols, while line-scoped conservative server roles remove section/intro overclassification. A final structural rule recognizes only existing colon-terminated enumeration leads; the provider-free floor passes all hard gates and thresholds at 0.7970/0.6930/0.7917 across primary/generalization-7/generalization-9. The private diff remains owner-only. `holdout_10` remains sealed and unread.
...
The role-constrained production contract and deterministic regressions are green. Every authorized tuning floor now exceeds 0.65 without server invention of optional code/bold/semantic accents. The remaining gate is one renewed primary model benchmark under a new owner budget; only a primary pass may unlock one untouched holdout operation. No model or holdout quality claim is made.
