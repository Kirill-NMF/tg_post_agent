...
| Phase 12: Real Formatting Adapter | Stage 3 | Option 2 technical gate green; authoritative post-fix primary failed closed on a role-to-emoji schema gap now fixed offline; quality benchmark not passed | 3/3 | Shared dynamic ID and role-emoji schema tests, seven-segment exact-job canary, lexical/emoji/final/export checks, private gold round-trip/evaluator | Keep untouched holdout sealed; no further benchmark call without a new explicit decision. |
...
The category-only ledger is 49/50. The authoritative post-fix primary verification used exactly one Claude Sonnet/OpenRouter attempt with fallback disabled and no retry. Provider transport succeeded, but the semantic gate rejected a role-incompatible emoji insertion. The provider and local parser now share exact request-specific segment IDs and role-to-emoji variants. No candidate, private diff, hard-gate result, or model-quality score exists. `holdout_10` remains sealed because primary did not pass.
...
Tier 2 draft-generation delivery passed with one terminal draft result and cleanup. Its historical Stage 2 bounded ledger closed at 15/15; the current cross-stage ledger is 49/50. The remaining Stage 2 gate is owner 3/3 acceptance.
