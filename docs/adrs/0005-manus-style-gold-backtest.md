...
## Final primary and sealed-holdout result

Commit `fb78e38` closes the remaining drift by generating `list_marker` schema branches only for canonical `nested_list` IDs and only for the dash variant already present in source punctuation. The provider-visible schema and local Ajv parser consume this same request-specific contract; the semantic role/source checks remain defense in depth. RED fixtures proved that the prior schema admitted both a non-nested target and a source-incompatible marker.

One post-fix primary attempt then passed shared shape and semantic validation with no retry or fallback. Every hard gate passed: lexical sequence, punctuation, balanced Telegram Markdown, forbidden styles/emoji categories, and invented hashtag/CTA/question checks. Its weighted style score was `0.6772`, above the fixed `0.65` threshold. Role F1 was `1.0` for heading, section, bold, paragraph, main heading, section heading, intro, audience question, hashtag footer, paragraph boundary, and semantic accent; prompt/code F1 was `0.6667`; list F1 was `0`; emoji-role F1 was `0.3889`.

Only after that immutable pass gate was written did the runner unseal `holdout_10`. No prompt, schema, parser, renderer, evaluator, or threshold changed between calls. The single holdout attempt also passed shape and semantics with no retry or fallback. All hard gates passed and the weighted style score was `0.7646`. Heading, section, paragraph, main heading, section heading, nested list, hashtag footer, paragraph boundary, and semantic accent F1 were `1.0`; list F1 was `0.8889`; primary-list F1 was `0.8`; bold F1 was `0.0606`; emoji-role F1 was `0.5128`. Zero-applicable-role metrics remain reported as zero rather than being inferred as literary failures.

Both generated candidates and human-readable diffs remain private, gitignored, and mode `0600`. The two-step protocol used exactly two provider attempts and ended at ledger `55/60`. This closes the deterministic benchmark gate, not the owner's literary/UX acceptance.

## Owner-visible product correction

The owner review proved that the similarity benchmark was product-invalid despite its green score: the delivered post could retain Markdown headings, use italic prose, omit required CRYPTUS_MEDIA list anchors, and add an unapproved expressive emoji. A weighted score can no longer override an explicit product-rule failure.

Active Option 2 now uses one versioned fixed CRYPTUS_MEDIA prompt and asks Claude Sonnet for the final Telegram-ready text in one non-streaming call. The production path no longer asks the provider for the historical structured operation plan. A local fail-closed validator runs before persistence or notification and requires the scroll-plus-bold title, forbids Markdown headings and italic/strike/spoiler syntax, permits only the documented role emoji, verifies their line roles, replaces source list markers only as reversible presentation anchors, and compares every source word and punctuation token in order with heading case as the sole scoped case exception. The final source question must remain the final bold arrow line, and the owner fixture's `Раньше`/`Сейчас` comparison requires two orange list anchors.

The fixed structural contract is an engineering gate. Passing it does not establish literary or visual acceptance; the owner still reviews the private candidate. Any future similarity score is diagnostic only until this explicit contract passes.

The first controlled call at `fd4ed70` used exactly one Claude Sonnet/OpenRouter attempt with `maxAttempts=1`, no retry, and no fallback. The raw candidate was persisted privately before validation. It preserved the complete source word and punctuation sequence, used the required title, contained no Markdown heading, italic, wrapper, or unapproved emoji, but failed closed with `FORMAT_OPTION2_EMOJI_ROLE_INVALID`. Safe offline diagnostics found an allowed emoji away from a line-leading role and a CTA marker whose line was not wholly bold; the required orange `Раньше`/`Сейчас` anchors and final bold arrow question were also absent. No delivery occurred. Ledger: `56/60`. This is evidence that the new gate catches the exact product-invalid class rather than a successful owner acceptance.

Commit `f6e0da9` records the evidence-backed correction as prompt version `cryptus_media_option2_v2`: paired `Раньше:`/`Сейчас:` comparisons must be separate orange lines with only the labels bold, while a final direct question has exclusive `➡️` precedence and may not share its line with `🔥`. Deterministic RED fixtures cover missing comparison anchors, extra comparison-value bold, adjacent and Markdown-separated terminal-role conflicts, and two lexically identical accepted forms.

One post-fix owner-fixture call then passed the complete explicit contract in one Claude Sonnet/OpenRouter attempt with no retry or fallback. Words, punctuation, and order were exact; title, comparison labels, exclusive final question, emoji allowlist/roles, Markdown, wrapper, heading, and italic gates were all green. The candidate remains private and mode `0600` for literary/visual review. Ledger: `57/60`. This closes the corrected structural gate only; subjective owner acceptance remains open.
