...
export function markProviderBoundaryEntered(report) { return { ...report, providerStarted: true, providerAttempted: true }; }
...
async function main(){
 ...
 const report={markerFingerprint,processed:false,category:null,preflightMaxProviderAttempts:1,providerStarted:false,providerAttempted:false};
 try { const fixture=await projects.findById(state.projectId); const job=await jobs.findById(jobId); const category=exactPreflight({project:fixture?{id:fixture.id,fixtureFingerprint:fixture.messages.find(m=>m.kind==="command")?.text,state:fixture.state}:undefined,job,markerFingerprint}); if(category||fixture?.state!=="draft_generating")throw Error(category??"fixture_state_invalid"); const handlers=createAudioPipelineHandlers({config,projects,jobs}); Object.assign(report, markProviderBoundaryEntered(report)); const result=await new JobWorker(jobs,{GENERATE_DRAFT:handlers.GENERATE_DRAFT}).processOne({workerId:"single-stage-runner",expectedJobId:jobId,now:controlledNow}); report.processed=result.processed;report.category=result.processed?result.status:"no_job"; } catch(e){report.category=e instanceof Error?e.message:"runner_failure";} finally {await pool.end();await atomicReport(reportPath,report);} console.log(JSON.stringify(report));
}
